...
**테이블·컬럼 전체 목록은 [`docs/tables.md`](docs/tables.md) 에 있다** (앱 테이블 30개, 실제 스키마에서 뽑음).
...
- [ ] **상품별·카테고리별 통계는 아직 실시간 집계다** — 상위 N개를 뽑는 형태라 미리 접으려면 상품×날짜 테이블이 따로 필요하다. 일별 매출만 먼저 옮겼다
- [x] ~~통계 금액 설명~~ — 완료. '정산 내역' 블록으로 상품합계 − 할인 + 배송비 = 결제매출 − 환불 = 순매출을 화면에서 따라 읽게 함
- [ ] **표시 시간대가 UTC 그대로다** — 화면의 주문일시·배송일시가 전부 UTC 문자열이다. 통계의 날짜 경계만 `config('shop.timezone')` 로 맞춰뒀다. 화면 전반의 KST 변환은 따로 정리 필요
